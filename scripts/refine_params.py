...
sys.path.append('.')
sys.path.append('..')
from src.utils.params import param_utils
import numpy as np
from common import *
from scenes import *
import pyngp as ngp
...
image_dir = os.path.join(base_path, "image", "segmented")

if args.separate_calib:
    params_path = os.path.join(base_path, "calib", "params.txt")
else:
    params_path = os.path.join(args.root_dir, "calib", "params.txt")
...
for idx, param in enumerate(params):
    ...
    if (len(img_list)==0):
        img_list = natsorted(glob(f"{image_dir}/{param['cam_name']}*.[jp][pn]g"))
        print(image_dir, param['cam_name'])
